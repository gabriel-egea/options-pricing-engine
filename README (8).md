# options-pricing-engine

European & exotic option pricer: Black-Scholes, binomial tree, Monte Carlo, Greeks.

A from-scratch implementation of option pricing methods, built to make the
underlying mathematics explicit rather than hiding it behind a library call.

### Methods at a glance

| Method | What it prices best | Key idea |
|---|---|---|
| Black-Scholes | European options | closed-form formula |
| Binomial tree (CRR) | American options | backward induction with early-exercise |
| Monte Carlo | exotic / path-dependent options | simulate and average payoffs |

All three are cross-validated against each other (see below).

---

## Status / roadmap

- [x] **Black-Scholes** closed-form pricing of European calls & puts (with dividend yield)
- [x] **Greeks**: delta, gamma, vega, theta, rho (analytical)
- [x] **Greeks visualization** (curves vs spot and vs maturity)
- [x] **Binomial tree** pricing (European & American), converging to Black-Scholes
- [x] **Monte Carlo** pricing (antithetic variates, confidence interval, convergence)
- [x] **Exotic options** (Asian, barrier) via Monte Carlo, with validation
- [x] **Implied volatility** solver + volatility skew (plus a script for live market data)

This README is updated as each block is added.

---

## The model

Under Black-Scholes-Merton, the underlying price $S$ follows a geometric
Brownian motion:

$$dS = (r - q)\,S\,dt + \sigma\,S\,dW$$

The price of a European **call** and **put** is:

$$C = S e^{-qT} N(d_1) - K e^{-rT} N(d_2)$$
$$P = K e^{-rT} N(-d_2) - S e^{-qT} N(-d_1)$$

with

$$d_1 = \frac{\ln(S/K) + (r - q + \tfrac{1}{2}\sigma^2)T}{\sigma\sqrt{T}}, \qquad d_2 = d_1 - \sigma\sqrt{T}$$

where $N(\cdot)$ is the standard normal CDF, $r$ the risk-free rate, $q$ the
dividend yield, $\sigma$ the volatility and $T$ the time to maturity.

The **Greeks** measure how the price reacts to each input: delta to the spot,
gamma to delta, vega to volatility, theta to time, rho to the rate.

---

## Project structure

```
options-pricing-engine/
├── black_scholes.py   # core pricing + Greeks
├── demo.py            # runnable example (prices, Greeks, parity check, plot)
├── greeks_plots.py    # plots of the Greeks (vs spot and vs maturity)
├── binomial.py        # CRR binomial tree (European & American)
├── binomial_demo.py   # convergence to BS + early-exercise premium
├── monte_carlo.py     # Monte Carlo pricer (antithetic + confidence interval)
├── monte_carlo_demo.py # MC vs BS + convergence study
├── exotics.py         # path-dependent options (Asian, barrier)
├── exotics_demo.py    # exotic pricing + validations
├── implied_vol.py     # implied volatility solver (Newton + Brent)
├── implied_vol_demo.py # validation + volatility skew plot
├── compare_methods.py # capstone: the 3 methods side by side
├── market_smile.py    # real market smile via yfinance (optional)
├── requirements.txt   # dependencies
└── README.md
```

## Installation

```bash
pip install -r requirements.txt
```

## Usage

```python
import black_scholes as bs

price = bs.bs_price(S=100, K=100, T=1, r=0.05, sigma=0.20, option_type="call")
print(price)            # 10.4506

g = bs.greeks(S=100, K=100, T=1, r=0.05, sigma=0.20, option_type="call")
print(g["delta"])       # 0.6368
```

Run the full demonstration:

```bash
python demo.py
```

Example output:

```
Call price : 10.4506
Put  price : 5.5735

Greeks (call):
  delta = 0.6368
  gamma = 0.0188
  vega  = 37.5240
  theta = -6.4140
  rho   = 53.2325

Put-call parity check: match: True
```

![Option value vs underlying price](bs_price_vs_spot.png)

---

## Greeks visualization

Generated by `greeks_plots.py`.

The four core Greeks as a function of the underlying price:

![Greeks vs spot](greeks_vs_spot.png)

- **Delta** runs from 0 to 1 for a call (0 to -1 for a put).
- **Gamma** and **vega** are bell-shaped and peak at-the-money.
- **Theta** (time decay) is most negative around the money.

Effect of time to maturity:

![Greeks by maturity](greeks_by_maturity.png)

- **Gamma** sharpens into a tall, narrow peak at-the-money as expiry approaches.
- **Vega** grows with maturity: longer-dated options are more sensitive to volatility.

---

## Binomial tree (Cox-Ross-Rubinstein)

Time is split into `N` steps; at each step the underlying moves up by
$u = e^{\sigma\sqrt{\Delta t}}$ or down by $d = 1/u$. Pricing is done by
backward induction under the risk-neutral probability

$$p = \frac{e^{(r-q)\Delta t} - d}{u - d}$$

Unlike Black-Scholes, the tree prices **American** options: at every node it
takes the maximum of the continuation value and the immediate-exercise value.

```python
from binomial import binomial_price

binomial_price(S=100, K=100, T=1, r=0.05, sigma=0.20, N=1000,
               option_type="put", exercise="american")   # 6.0896
```

**Results from `binomial_demo.py`:**

- European call, binomial `N=1000` = 10.4486 vs Black-Scholes 10.4506 (converges).
- American put = 6.0896 vs European put 5.5715 -> **early-exercise premium ~0.52**.
- American call (no dividend) = European call: early exercise is never optimal.

![Binomial convergence](binomial_convergence.png)

The binomial price oscillates around and converges to the Black-Scholes value
as the number of steps increases.

---

## Monte Carlo

The risk-neutral terminal price of the underlying is simulated as

$$S_T = S_0 \exp\!\Big( (r - q - \tfrac{1}{2}\sigma^2)T + \sigma\sqrt{T}\,Z \Big), \quad Z \sim \mathcal{N}(0,1)$$

and the option price is the discounted average payoff over many simulations.
The estimate comes with a **standard error** and a **95% confidence interval**;
**antithetic variates** are used to reduce variance.

```python
from monte_carlo import mc_price_european

price, stderr, ci = mc_price_european(S=100, K=100, T=1, r=0.05,
                                      sigma=0.20, M=200_000, option_type="call")
```

**Results from `monte_carlo_demo.py`:**

- European call, MC (M=200k) = 10.4763, 95% CI [10.43, 10.52] -> contains Black-Scholes 10.4506.
- Antithetic variates cut the standard error by ~29% at equal M.
- The error decreases like $1/\sqrt{M}$ (to halve it, run 4x more paths).

![Monte Carlo convergence](monte_carlo_convergence.png)

---

## Exotic options (path-dependent, Monte Carlo)

Path-dependent options have no closed form in general, so full price paths are
simulated:

$$S_{t+\Delta t} = S_t \exp\!\Big( (r - q - \tfrac{1}{2}\sigma^2)\Delta t + \sigma\sqrt{\Delta t}\, Z \Big)$$

**Asian options** pay on the *average* price over the path (cheaper than a
vanilla, since averaging smooths volatility). **Barrier options** activate
(*knock-in*) or expire (*knock-out*) when the price touches a level `B`.

```python
from exotics import asian_option, barrier_option

asian_option(S=100, K=100, T=1, r=0.05, sigma=0.20, option_type="call",
             average="arithmetic")
barrier_option(S=100, K=100, T=1, r=0.05, sigma=0.20, B=130,
               option_type="call", barrier_type="up-and-out")
```

**Validations from `exotics_demo.py`:**

- Vanilla priced from the path engine matches Black-Scholes (engine check).
- Geometric Asian: Monte Carlo matches its **exact closed form** (5.5962 vs 5.5939).
- Averaging lowers the price: geometric <= arithmetic Asian <= vanilla.
- **In-out parity is exact**: up-and-out (3.57) + up-and-in (6.91) = vanilla (10.48).

![Sample paths with barrier](barrier_paths.png)

*Caveat:* the barrier is monitored at discrete steps, which slightly
underestimates the knock-out probability versus a continuously-monitored
barrier; more steps reduce this discretization bias.

---

## Implied volatility & the skew

Given a *market price*, the **implied volatility** is the sigma that, plugged
into Black-Scholes, reproduces that price. Since the price is strictly
increasing in sigma (vega > 0), a unique solution exists; it is found with
**Newton-Raphson** (using vega as the derivative) and a robust **Brent**
fallback.

```python
from implied_vol import implied_vol

implied_vol(price=10.45, S=100, K=100, T=1, r=0.05, option_type="call")  # ~0.20
```

If Black-Scholes were exactly right, implied vol would be the **same for every
strike** (a flat line). In reality, plotting it against strike gives a **smile**
(FX) or a **skew** (equity indices: higher vol for low strikes). This reveals
that the market prices in **fat tails and crash risk** -- it does not believe in
the constant-volatility assumption.

![Volatility skew](vol_smile.png)

`market_smile.py` builds the same plot from **live option data** (needs
`pip install yfinance` and internet), computing the implied vol of a real
option chain with our own solver.

---

## Cross-method validation (capstone)

`compare_methods.py` prices the same options three independent ways and checks
they agree -- strong evidence the whole library is correct.

| Strike | Black-Scholes | Binomial | Monte Carlo |
|---|---|---|---|
| 80  | 24.589 | 24.589 | 24.581 |
| 100 | 10.451 | 10.449 | 10.431 |
| 120 |  3.247 |  3.248 |  3.237 |

Largest disagreement across all strikes: **0.02** (Monte Carlo noise).

![Three methods agree](methods_comparison.png)

## How to run

```bash
pip install -r requirements.txt
python demo.py                 # Black-Scholes prices, Greeks, parity check
python greeks_plots.py         # Greeks curves
python binomial_demo.py        # binomial convergence + American options
python monte_carlo_demo.py     # Monte Carlo convergence
python exotics_demo.py         # Asian & barrier options
python implied_vol_demo.py     # implied volatility & skew
python compare_methods.py      # the three methods side by side
```

---

## Notes

- Greeks are returned in their raw (per-unit) form. Common conventions:
  vega per +1% of vol = `vega / 100`, theta per day = `theta / 365`.
- Put-call parity ( $C - P = S e^{-qT} - K e^{-rT}$ ) is used in `demo.py`
  as an automatic correctness check.
